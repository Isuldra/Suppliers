  ...
  async (_, payload: Array<{ to: string; subject: string; html: string }>) => {
    ...
    try {
      ...
      for (let i = 0; i < payload.length; i++) {
        ...
        const subjectFile = path.join(os.tmpdir(), `onemed-batch-subject-${Date.now()}-${i}.txt`);
        ...
        psProcess.on('close', (code: number | null) => {
          ...
          if (resultsMatch) {
            try {
              ...
              log.info(`Batch sending completed: ${successCount} success, ${failCount} failed`);
...
ipcMain.handle(
  ...
  async (_, payload: { to: string; subject: string; html: string }) => {
    ...
    try {
      ...
      tempHtmlFilePath = path.join(os.tmpdir(), `onemed-outlook-${Date.now()}.html`);
      ...
      tempSubjectFilePath = path.join(os.tmpdir(), `onemed-subject-${Date.now()}.txt`);
...
`;
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      log.error('Automatic email sending error (outer try-catch):', errorMessage);
      ...
      if (tempHtmlFilePath) {
        try {
          ...
          log.warn('Outer catch: Failed to cleanup temp HTML file', cleanupError);
        }
      }
      ...
      if (tempSubjectFilePath) {
        try {
          ...
          log.warn('Outer catch: Failed to cleanup temp subject file', cleanupError);
        }
      }
