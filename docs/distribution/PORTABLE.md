# Pulse - Portable Version

This document explains the portable version of Pulse for Windows.

## What is the Portable Version?

The portable version (`Pulse-Portable.exe`):
...
3. Run the standard portable build script:
   ...
   ```
4. The portable executable (`Pulse-Portable.exe`) will be created in the `release` directory (based on `package.json` config).
...
1. Share the single `Pulse-Portable.exe` file (generated by `npm run dist:portable`) with users.
2. Users can place this `.exe` file anywhere they have execute permissions (e.g., Desktop, Documents, USB drive).

## Instructions for Users

1. Place the `Pulse-Portable.exe` file in a convenient location.
