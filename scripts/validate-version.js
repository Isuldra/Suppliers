...
if (isPackageNewer) {
  ...
  console.log("   This means you've updated package.json but haven't created a git tag.");
  console.log('\n📋 Required actions:');
  console.log('   1. Commit any pending changes');
  console.log(
    '   2. Create git tag: git tag -a v' + packageVersion + " -m 'Release v" + packageVersion + "'"
  );
  ...
  console.log(`   Both package.json and git tag are at version ${packageVersion}`);
  ...
  try {
    ...
    if (currentCommit === tagCommit) {
      ...
      console.log('   Consider creating a new version if you want to release these changes');
    }
  } catch (error) {
