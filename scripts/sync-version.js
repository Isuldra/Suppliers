...
async function checkNotDetachedHead() {
  try {
    ...
    throw new Error(
      "Detached HEAD state detected. Please checkout a branch before versioning."
    );
  }
}
...
async function syncVersionWithGitTag() {
  try {
    ...
    log(
      `Updating package.json version from ${currentVersion} to ${version}`,
      "process"
    );
    ...
    if (!isDryRun) {
      ...
      log(
        `[DRY-RUN] Would update package.json version to ${version}`,
        "warning"
      );
    }
    ...
    try {
      ...
      if (error.message.includes("nothing to commit")) {
        ...
        log(
          "Could not commit version sync (this is normal if no changes were made)",
          "warning"
        );
      }
    }
...
async function createNewVersion(versionType = "patch") {
  try {
    ...
    if (isDryRun) {
      log(
        `[DRY-RUN] Would bump version from ${currentVersion} (type: ${versionType})`,
        "warning"
      );
      return null;
    }
  ...
  try {
    ...
    try {
      ...
      if (packageJson.version !== version) {
        log(
          "Version mismatch detected. Run 'sync' to update package.json",
          "warning"
        );
      }
    } catch (error) {
