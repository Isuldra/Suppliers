...
!macro customInstall
  ...
  ; Ensure Start Menu shortcuts are created (electron-builder should do this, but we make it explicit)
  CreateDirectory "$SMPROGRAMS\Pulse"
  CreateShortCut "$SMPROGRAMS\Pulse\Pulse.lnk" "$INSTDIR\Pulse.exe" "" "$INSTDIR\Pulse.exe" 0
  CreateShortCut "$SMPROGRAMS\Pulse\Uninstall Pulse.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
  ...
  ; Using productName from package.json: "Pulse"
  ; Changed from HKLM to HKCU since this is a per-user installation (perMachine: false)
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\Pulse" \
    "DisplayName" "Pulse"
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\Pulse" \
  ...
  WriteRegDWORD HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\Pulse" \
    "NoModify" 1
  WriteRegDWORD HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\Pulse" \
    "NoRepair" 1
  
  ; Add application to startup (optional, uncomment if needed)
  ; WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "Pulse" "$INSTDIR\Pulse.exe"
!macroend
